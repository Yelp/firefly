...
class AggregatingDataSourceTest(T.TestCase):
	...
	def test_init_ds_args(self):
		...
		data_source = AggregatingDataSource(data_sources=self.test_data_sources)
		T.assert_equal(data_source.data_sources, expected_data_sources)
	...
	def test_list_path_no_path(self):
		...
		expected_paths = [{
			...
			'type': 'dir'
		} for i in xrange(len(data_source.data_sources))]

		with mock.patch.object(data_source, '_request_paths_from_ds') as mock_request_paths:
			mock_request_paths.side_effect = [[path] for path in expected_paths]

			actual_paths = data_source.list_path(test_path)

			T.assert_equal(mock_request_paths.call_count, len(data_source.data_sources))

			for ds in data_source.data_sources:
				mock_request_paths.assert_any_call(ds, test_path)

		T.assert_equal(expected_paths, actual_paths)

		for expected_path, expected_data_source in zip(expected_paths, data_source.data_sources):
			T.assert_in(expected_path['name'], data_source.key_mapping_cache)
			T.assert_equal(data_source.key_mapping_cache[expected_path['name']], expected_data_source)
	...
	def test_list_path(self):
		...
		with mock.patch.object(data_source, '_request_paths_from_ds') as mock_request_paths:
			with mock.patch.object(data_source, '_data_source_for_stat_key') as mock_ds_for_stat_key:
				mock_ds_for_stat_key.return_value = data_source.data_sources[1]
				mock_request_paths.return_value = [path for path in expected_paths]

				actual_paths = data_source.list_path(test_path)

				mock_ds_for_stat_key.assert_called_once_with(test_path[0])
				mock_request_paths.assert_called_once_with(data_source.data_sources[1], test_path)
	...
	def test_data_single_from_single_source(self):
		...
		data_source = AggregatingDataSource(data_sources=self.test_data_sources)
		test_data_source = data_source.data_sources[2]
		...
		with mock.patch.object(data_source, '_data_source_for_stat_key') as mock_ds_for_stat_key:
			with mock.patch.object(data_source, '_request_data_from_ds') as mock_request_data:
				mock_ds_for_stat_key.return_value = test_data_source
				mock_request_data.return_value = dummy_data

				actual_data = data_source.data(test_stat, test_start, test_end, test_width)
	...
	def test_data_multiple_from_single_source(self):
		...
		with mock.patch.object(data_source, '_data_source_for_stat_key') as mock_ds_for_stat_key:
			with mock.patch.object(data_source, '_request_data_from_ds') as mock_request_data:
				...
				actual_data = data_source.data(test_stats, test_start, test_end, test_width)
	...
	def test_data_multiple_from_multiple_sources(self):
		...
		"""
		data_source = AggregatingDataSource(data_sources=self.test_data_sources)
		# A -> 0, B -> 1, C -> 2 in data_source.data_sources
		test_stats = [
			['src.MajorSourceA', 'key.minorStat1', 'variant.Blue', '50th_percentile'],
		...
		with mock.patch.object(data_source, '_data_source_for_stat_key') as mock_ds_for_stat_key:
			with mock.patch.object(data_source, '_request_data_from_ds') as mock_request_data:
				...
				mock_request_data.assert_any_call(data_source.data_sources[0],
					[test_stats[0], test_stats[2]], test_start, test_end, test_width)
				mock_request_data.assert_any_call(data_source.data_sources[1],
					[test_stats[1]], test_start, test_end, test_width)
				mock_request_data.assert_any_call(data_source.data_sources[2],
	...
	def test_int_dict_to_list(self):
		...
		actual_list = data_source._int_dict_to_list(test_dict)
	...
	def test_request_paths_from_ds(self):
		...
		"""
		data_source = AggregatingDataSource(data_sources=self.test_data_sources)
		test_data_source = data_source.data_sources[0]
		test_path = ['src.A']
		...
		with self._patch_urlopen() as mock_urlopen:
			...
			actual_paths = data_source._request_paths_from_ds(test_data_source, test_path)
	...
	def test_request_paths_from_ds_url_error(self):
		...
		with self._patch_urlopen() as mock_urlopen:
			with mock.patch.object(data_source, 'logger') as mock_logger:
	...
	def test_request_data_from_ds(self):
		...
		with self._patch_urlopen() as mock_urlopen:
			...
			returned_data = data_source._request_data_from_ds(test_data_source,
				test_sources,
				test_start,
				test_end,
				test_width)
	...
	def test_data_source_for_stat_key_in_cache(self):
		...
		"""
		data_source = AggregatingDataSource(data_sources=self.test_data_sources)
		expected_data_source = data_source.data_sources[2]

		stat_key = 'src.test_stat_key'

		data_source.key_mapping_cache[stat_key] = expected_data_source

		with mock.patch.object(data_source, '_find_data_source_for_stat_key') as mock_find_ds_for_stat:
			actual_ds = data_source._data_source_for_stat_key(stat_key)
	...
	def test_find_data_source_for_stat_key(self):
		...
		with mock.patch.object(data_source, '_request_paths_from_ds', fake_paths_from_ds):
			actual_ds = data_source._find_data_source_for_stat_key(test_key)

		T.assert_equal(expected_data_source, actual_ds)
		T.assert_in(test_key, data_source.key_mapping_cache)
		T.assert_equal(expected_data_source, data_source.key_mapping_cache[test_key])
